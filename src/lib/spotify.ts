...
export function addToCreationQueues(entity: keyof typeof creationQueues, id: string) {
...
async function retrySpotifyCall<T>(fn: () => Promise<T>, operationName: string): Promise<{ error?: any, data?: T }> {
    let attempts = 0;
    ...
    while (attempts < SPOTIFY_RETRY_LIMIT) {
        try {
            ...
            if (status === 429) {
                const retryAfter = typeof err?.response?.headers?.get === "function"
                ...
                logger.error({ operation: operationName, error: err }, `Spotify API error in ${operationName}`);
                return { error: err };
            }
        }
    }
...
export async function createGenres() {
    const existingGenres = await tryCatch(db.genre.findMany({
    ...
    const createdGenres = await tryCatch(db.genre.createManyAndReturn({
...
export async function createArtists(spotify: SpotifyApi) {
    const existingArtist = await tryCatch(db.artist.findMany({
    ...
    await createGenres();
    const results = await Promise.all(newArtists.map(artist => {
        return tryCatch(db.artist.create({
            data: {
                spotifyId: artist.id, name: artist.name, image: artist.images[0]?.url,
                genres: { create: artist.genres.map(genre => ({ genre: { connect: { name: genre } } })) },
            },
        }));
    }));
...
export async function createAlbums(spotify: SpotifyApi) {
    const existingAlbums = await tryCatch(db.album.findMany({
    ...
    for (const batchIds of batches) {
        ...
        for (const album of albumsData ?? []) {
            const createdAlbum = await tryCatch(db.album.create({
                data: {
                    spotifyId: album.id, name: album.name, image: album.images[0]?.url,
                    artists: { create: album.artists.map(artist => ({ artist: { connect: { spotifyId: artist.id } }, role: "primary" })) },
                },
            }));
...
export async function createTracks(spotify: SpotifyApi) {
    const existingTracks = await tryCatch(db.track.findMany({
    ...
    for (const batchIds of batches) {
        ...
        for (const track of tracksData ?? []) {
            const createdTrack = await tryCatch(db.track.create({
                data: {
                    spotifyId: track.id,
                    name: track.name,
                    ...
                    artists: {
                        create: track.artists.map((artist, index) => ({ artist: { connect: { spotifyId: artist.id } }, role: index === 0 ? "primary" : "featured" })),
                    }
                },
            }));
...
export async function createPlaybacks(userId: string, playbacks: PlayHistory[]) {
    for (const playback of playbacks) {
        const createdPlayback = await tryCatch(db.playback.create({
            data: {
                user: {
                    connect: { id: userId },
                },
