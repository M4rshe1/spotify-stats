...
export function TrackTimeDistribution({
  trackId,
  period,
}: {
  trackId: number;
  ...
  const queryEnabled = Number.isFinite(trackId) && trackId > 0;

  const { data: result, isLoading } =
    api.chart.getTrackTimeDistribution.useQuery(
      {
  ...
  return (
    <SeriesChartCard
      title="This track by hour"
      description="When you played this track during the day"
      chartConfig={{
        trackHourPercent: {
          label: "Share by hour",
          color: "var(--chart-1)",
      ...
      <CategoryBarChart<HourDatum>
        ...
        formatTooltipValue={(value, payload) => (
          <div className="flex flex-col gap-1">
            ...
            <div>{formatPercent(value)} of this track in the period</div>
            <div>
